...
//		}
	...
	boolean CheckId(String id) {
		...
		if (id.startsWith("INST")) {
			type = 1;
			try {
				stmt = dbc.getConnection().createStatement(); // Use getConnection()
				logid = Integer.parseInt(id.substring(4));
	
				System.out.println("Inst Log id : " + logid);
				String qry = "SELECT iid FROM INSTRUCTOR WHERE iid = " + logid;
			...
			try {
				...
				String qry = "SELECT sid FROM STUDENT WHERE sid = " + logid;
				ResultSet rs = stmt.executeQuery(qry);
				if (rs.next()) {
					flag = true;
				}
	...
	boolean checkPassword(String pass) {
		...
		if (type == 0) { // Student
			try {
				stmt = dbc.getConnection().createStatement(); // Use getConnection()
				String qry = "SELECT password FROM STUDENT WHERE sid = " + logid;
				ResultSet rs = stmt.executeQuery(qry);
			...
			try {
				stmt = dbc.getConnection().createStatement(); // Use getConnection()
				String qry = "SELECT password FROM INSTRUCTOR WHERE iid = " + logid;
				ResultSet rs = stmt.executeQuery(qry);
				if (rs.next() && rs.getString(1).equals(pass)) {
					flag = true;
				}
			...
			if (pass.equals("admin")) flag = true;
		}
